...
async fn do_it(cache_dir: &Path, song: &VideoLink, player: Weak<Mpv>) {
    ...
    let path = match download(cache_dir.join("m").join("preemptive-dl"), song, false).await {
    ...
    loop {
        ...
        let Ok(from_position) = player.simple_prop::<i64>("playlist-count") else {
            return;
        };
        tracing::debug!(?from_position);
        let to = {
        ...
        tracing::debug!("removing old song");
        if let Err(e) = player.playlist_remove_index(to) {
            tracing::error!(error = ?e, "failed to remove the uncached version");
            return;
        };
        tracing::debug!("moving new one");
        if let Err(e) = player.playlist_move(from_position as _, to) {
